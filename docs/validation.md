...
- Sau khôi phục tám nguồn đã đối chiếu bản gốc, bốn lượt hỏi lại qua `gpt-5.5` và `claude-sonnet-5` đều nhận được nguồn mong đợi và luồng SSE `reasoning → searching → generating → answer`. Hai lượt GPT nêu đúng giới hạn về lịch cá nhân và đầu mối khôi phục mật khẩu; lượt Sonnet hỏi lịch vẫn tự khẳng định LMS/cổng hiển thị lịch cá nhân dù căn cứ chưa xác nhận. Ghi nhận đây là lỗi nội dung còn phải đánh giá, không tính việc có trích dẫn là đủ để chấm câu trả lời đúng. Các lượt này dùng chế độ hội thoại tạm thời, không lưu vào lịch sử sinh viên.
- Vòng sửa tiếp theo dùng dấu vân tay HTML `html-v1`: nội dung hiển thị và liên kết cả menu, bỏ trạng thái form/script/style và span bộ đếm lượt xem độc lập. Đã đối chiếu hai bản tải thật: byte HTML và bộ đếm khác nhau nhưng dấu vân tay nội dung giống nhau. PDF giữ hash byte; HTML chuyển từ hash cũ phải duyệt lại một lần. 17/17 ca ingestion/security đạt, bao gồm đổi URL/menu, ngày/phí, giữ nguồn khi chỉ đổi trạng thái form và không tự duyệt phiên bản mới. Trước vòng fingerprint này, toàn bộ 140/140 kiểm thử backend đạt.
