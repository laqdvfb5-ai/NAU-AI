...
export class Database implements StudentDataProvider {
  ...
  async initialize() {
    const statements = [
      ...
      "UPDATE api_profiles SET last_success_revision=revision WHERE last_success_revision IS NULL AND last_test->>'ok'='true' AND last_test->>'kind'='completion' AND last_test->>'revision'=revision::text",
